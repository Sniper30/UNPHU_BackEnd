...
const { Book, Vote } = require("../database/model/model");
...
const Postcontroll = {
    //Post
     ...
     voting: async (req, res) => {
         ...
         Vote.find({_id:docs._id}).then(r => res.send(r));

         });
     }
}

     router.get("/gettingvotes/:bookid", (req, res) => Postcontroll.get(req, res));

     router.put("/update/:bookid", (req, res) => Postcontroll.voting(req, res));

     router.post("/adding", (req, res) => Postcontroll.vote(req, res) );
